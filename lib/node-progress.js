...
function ProgressBar (fmt, options) {
  ...
  this.renderThrottle = options.renderThrottle !== 0 ? (options.renderThrottle || 250) : 0
...
ProgressBar.prototype.render = function (tokens, force) {
  ...
  const eta = (Math.floor(percent) === 100) ? 0 : elapsed * (this.total / this.curr - 1)
  ...
  if (isNaN(elapsed) || !isFinite(elapsed)) {
    str = str.replace(':elapsed', '--s')
  } else if (elapsed / 1000 > 60) {
    const min = ((elapsed / 1000) / 60).toFixed(0)
    const sec = Math.round((elapsed / 1000) % 60)
    str = str.replace(':elapsed', min + 'm' + ((sec > 9) ? sec : '0' + sec) + 's')
  } else {
    const sec = Math.round(elapsed / 1000)
    str = str.replace(':elapsed', sec + 's')
  }

  if (isNaN(eta) || !isFinite(eta)) {
    str = str.replace(':eta', '--s')
  } else if (eta / 1000 > 60) {
    const min = ((eta / 1000) / 60).toFixed(0)
    const sec = Math.round((eta / 1000) % 60)
    str = str.replace(':eta', min + 'm' + ((sec > 9) ? sec : '0' + sec) + 's')
  } else {
    const sec = Math.round(eta / 1000)
    str = str.replace(':eta', sec + 's')
  }

  if (rate / 1000000 > 1) {
    str = str.replace(':rate', (rate / 1000000).toFixed(2) + ' Mb/s')
  } else if (rate / 1000 > 1) {
    str = str.replace(':rate', (rate / 1000).toFixed(2) + ' Kb/s')
  } else {
    str = str.replace(':rate', Math.round(rate) + ' B/s')
  }
